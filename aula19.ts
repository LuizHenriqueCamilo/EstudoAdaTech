...
class Pessoa {
  ...
  // Atributos são as propriedades do objeto, e métodos são as funções que o objeto pode executar.
  // Atributos
  nome!: string;
  idade: number;
  altura!: number;
...
// Uma interface é uma estrutura que define um contrato para classes. Ela especifica quais propriedades e métodos uma classe deve ter, mas não fornece a implementação deles. As classes que implementam a interface devem fornecer a implementação dos métodos definidos na interface. só esiste no TypeScript, não existe no JavaScript.
// o I na IPessoa é uma convenção para indicar que é uma interface. É uma boa prática nomear interfaces com um "I" no início do nome, mas não é obrigatório.
interface IPessoa {
  nome: string; // Removed definite assignment assertion
  idade: number;
  altura: number; // Removed initializer
  dormir(): void; // método que não retorna nada (void)
}
...
class Pessoa2 implements IPessoa {
  ...
  idade: number;
  altura!: number;
  //1. private: É uma palavra-chave que indica que a propriedade ou método só pode ser acessado dentro da própria classe. Isso significa que não pode ser acessado diretamente de fora da classe, o que ajuda a proteger os dados e a lógica interna da classe. Por exemplo, se tivermos um atributo "senha" em uma classe "Usuario", podemos torná-lo privado para evitar que seja acessado diretamente de fora da classe.
  private _cpf: string; // Renamed to avoid conflict with the getter.
  ...
  readonly dataNascimento: Date = new Date(); // atributo somente leitura, não pode ser alterado depois de inicializado. O modificador "readonly" indica que o atributo só pode ser lido, mas não pode ser modificado depois de inicializado.
  dormir(): void {
